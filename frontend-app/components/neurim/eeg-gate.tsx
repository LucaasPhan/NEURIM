...
import { AlertTriangle, Battery, Brain, Check, FlaskConical, RefreshCw, Wifi, WifiOff } from "lucide-react";
...
export function EegGate({ children }: { children: ReactNode }) {
  ...
  const unlocked = ready || bypassed;

  if (!unlocked) {
    return <EegGateScreen eeg={eeg} onSkip={() => setBypassed(true)} />;
  }
...
function EegGateScreen({ eeg, onSkip }: { eeg: EegStatusHook; onSkip: () => void }) {
  ...
  const connecting = retrying || state === "scanning";
  const calibrating = state === "calibrating";
  const fitting = state === "fitting" || state === "ready_to_calibrate";
  const validating = state === "device_validation";
...
function DeviceValidationView({ eeg }: { eeg: EegStatusHook }) {
  const device = eeg.status?.device;
  const virtual = Boolean(device?.is_virtual);
  return (
    <div className="flex flex-col items-center">
      <div className="grid h-16 w-16 place-items-center rounded-full bg-amber-500/15 text-amber-600">
        {virtual ? <FlaskConical size={27} /> : <AlertTriangle size={27} />}
      </div>
      <div className="mt-6">
        <StatusHeading
          title={virtual ? "Simulation device detected" : "Unsupported headset"}
          subtitle={virtual
            ? `${device?.name || device?.id || "Virtual Brainwear"} is generated by EMOTIV Launcher, not a physical EPOC X.`
            : `${device?.model || "This device"} cannot be used for a real EPOC X session.`}
        />
      </div>
      {virtual ? (
        <button type="button" onClick={() => eeg.setDemoMode(true)} disabled={eeg.retrying} className="mt-7 inline-flex items-center gap-2 rounded-full bg-amber-500 px-6 py-2.5 text-sm font-medium text-black disabled:opacity-60">
          <FlaskConical size={16} /> Enter demo mode
        </button>
      ) : null}
      <button type="button" onClick={eeg.retry} className="mt-4 text-xs text-muted-foreground hover:text-foreground">Rescan devices</button>
    </div>
  );
}

const EPOC_X_SENSORS = ["AF3", "AF4", "F7", "F8", "F3", "F4", "FC5", "FC6", "T7", "T8", "P7", "P8", "O1", "O2"];
const REQUIRED_SENSORS = new Set(["AF3", "AF4", "F7", "F8", "F3", "F4", "FC5", "FC6"]);

function FittingView({ eeg }: { eeg: EegStatusHook }) {
  const status = eeg.status!;
  const device = status.device;
  const quality = status.quality;
  const progress = quality.required_stable_seconds > 0
    ? Math.min(100, quality.stable_seconds / quality.required_stable_seconds * 100)
    : 100;
  return (
    <div className="flex flex-col items-center">
      <div className="flex w-full items-start justify-between text-left">
        <div>
          <p className="text-xs font-medium uppercase text-muted-foreground">{status.mode === "demo" ? "Simulation" : "Physical device"}</p>
          <h2 className="mt-1 font-serif text-2xl">{device?.name || device?.model || "EPOC X"}</h2>
          <p className="mt-1 font-mono text-[11px] text-muted-foreground">{device?.id}</p>
        </div>
        <span className={cn("rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase", status.mode === "demo" ? "bg-amber-500/20 text-amber-600" : "bg-approach/15 text-approach")}>{status.mode}</span>
      </div>
      <div className="mt-5 grid w-full grid-cols-3 gap-2 text-left text-xs">
        <Metric icon={<Battery size={14} />} label="Battery" value={device?.battery_percent == null ? "--" : `${Math.round(device.battery_percent)}%`} />
        <Metric icon={<Wifi size={14} />} label="Signal" value={quality.signal == null ? "--" : `${Math.round(quality.signal * 100)}%`} />
        <Metric icon={<Check size={14} />} label="Contact" value={quality.overall == null ? "--" : `${Math.round(quality.overall)}%`} />
      </div>
      <div className="mt-5 grid w-full grid-cols-7 gap-2">
        {EPOC_X_SENSORS.map((sensor) => {
          const value = quality.sensors[sensor] ?? -1;
          const good = value >= 2;
          return <div key={sensor} title={`${sensor}: ${value < 0 ? "no reading" : `${value}/4`}`} className={cn("grid aspect-square place-items-center rounded-full border text-[10px] font-semibold", good ? "border-emerald-500/40 bg-emerald-500/15 text-emerald-600" : "border-destructive/40 bg-destructive/10 text-destructive", REQUIRED_SENSORS.has(sensor) && "ring-1 ring-primary/30")}>{sensor}</div>;
        })}
      </div>
      <p className="mt-2 text-[10px] text-muted-foreground">Outlined channels drive FAA and must meet the threshold.</p>
      {quality.blocking_reasons.length ? (
        <div className="mt-4 w-full rounded-md bg-destructive/10 px-3 py-2 text-left text-xs text-destructive">{quality.blocking_reasons.join(" · ")}</div>
      ) : (
        <div className="mt-4 w-full">
          <div className="h-2 overflow-hidden rounded-full bg-secondary"><div className="h-full bg-approach transition-[width]" style={{ width: `${progress}%` }} /></div>
          <p className="mt-2 text-xs text-muted-foreground">Hold steady for {Math.max(0, Math.ceil(quality.required_stable_seconds - quality.stable_seconds))}s</p>
        </div>
      )}
      {quality.warnings.length ? <p className="mt-3 text-xs text-amber-600">{quality.warnings.join(" · ")}</p> : null}
      <button type="button" onClick={eeg.calibrate} disabled={!status.can_calibrate || eeg.retrying} className="mt-5 rounded-full bg-primary px-6 py-2.5 text-sm font-medium text-primary-foreground disabled:cursor-not-allowed disabled:opacity-40">Start calibration</button>
    </div>
  );
}

function Metric({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return <div className="rounded-md border border-border bg-secondary/40 p-2"><div className="flex items-center gap-1 text-muted-foreground">{icon}<span>{label}</span></div><p className="mt-1 font-mono text-sm text-foreground">{value}</p></div>;
}
...
function EegStatusPill({ eeg }: { eeg: EegStatusHook }) {
  ...
  switch (state) {
    case "ready":
      ...
      label = status?.mode === "demo" ? "Simulation - ready" : "EPOC X connected";
      break;
    case "scanning":
    ...
    case "device_validation":
      dot = "bg-destructive";
      label = status?.device?.is_virtual ? "Simulation detected" : "Unsupported headset";
      showRetry = true;
      break;
    case "fitting":
    case "ready_to_calibrate":
      dot = "bg-amber-500 animate-blink";
      label = status?.mode === "demo" ? "Simulation fitting" : "EPOC X fitting";
      break;
    case "calibrating":
      dot = "bg-amber-500 animate-blink";
      label = "Calibrating…";
